...
const AnimationContext = createContext<AnimationProviderType>({
    ...
    setAnimateHeaderFooter: () => null,
    animateGrid: 0,
    setAnimateGrid: () => null,
    animateBizerk: 0,
    setAnimateBizerk: () => null,
});

const BizerContext = createContext<BizerProviderType>({
    bizerkCounter: 0,
    setBizerkCounter: () => null,
    bizerkMode: 'off',
    setBizerkMode: () => null,
});
...
const BizerkProvider: FC<ProviderProps> = ({ children }) => {
    ...
    return <BizerContext.Provider value={animationCounters}>{children}</BizerContext.Provider>;
};
...
function useBizerkContext() {
    ...
    if (context === undefined) throw new Error('useBizerkContext must be used within a BizerkProvider');
